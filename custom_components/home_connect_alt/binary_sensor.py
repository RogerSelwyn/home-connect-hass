...
async def async_setup_entry(
    ...
    def add_appliance(appliance: Appliance) -> None:
        ...
        for key, status in appliance.status.items():
            device = None
            if (
                isinstance(status.value, bool)
                or conf.get_entity_setting(key, "type") == "Boolean"
    ...
    def is_on(self) -> bool:
        if self._key in self._appliance.status:
            if self.has_entity_setting("on_state"):
                return self._appliance.status[
                    self._key
                ].value == self.get_entity_setting("on_state")
...
class SettingsBinarySensor(EntityBase, BinarySensorEntity):
    ...
    def is_on(self):
        if self._key in self._appliance.settings:
            if self.has_entity_setting("on_state"):
                return self._appliance.settings[
                    self._key
                ].value == self.get_entity_setting("on_state")

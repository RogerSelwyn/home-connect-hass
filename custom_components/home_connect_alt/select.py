...
from .common import Configuration, EntityManager, InteractiveEntityBase, is_boolean_enum
from .const import (
    CONF_DELAYED_OPS,
    CONF_DELAYED_OPS_ABSOLUTE_TIME,
    CONF_DELAYED_OPS_DEFAULT,
            ...
            for program in appliance.available_programs.values():
                if program.options:
                    for option in program.options.values():
                        if conf.get_entity_setting(
                            option.key, "type"
                        ) == "DelayedOperation" and (
                            entry_conf[CONF_DELAYED_OPS] == CONF_DELAYED_OPS_DEFAULT
                            or not DelayedOperationTime.has_program_run_time(appliance)
                        ):
                            device = DelayedOperationSelect(
                                appliance, option.key, conf, option
                            )
                            # remove the TIME delayed operation entity if it exists
                            reg = async_get(hass)
                            time_entity = reg.async_get_entity_id(
                                "time", DOMAIN, device.unique_id
                            )
                            if time_entity:
    ...
    def options(self) -> list[str]:
        ...
        if self._appliance.available_programs:
            if self._conf[CONF_TRANSLATION_MODE] == CONF_TRANSLATION_MODE_SERVER:
                return [
                    program.name if program.name else program.key
        ...
        if current_program:
            if (
                ...
                return (
                    ...
                    and self._conf[CONF_TRANSLATION_MODE]
                    == CONF_TRANSLATION_MODE_SERVER
                    else current_program.key
                )
            CL.debug(
                _LOGGER,
                CL.LogMode.VERBOSE,
        ...
        try:
            if self._conf[CONF_TRANSLATION_MODE] == CONF_TRANSLATION_MODE_SERVER:
                program = next(
                    (
                        p
                        for p in self._appliance.available_programs.values()
                        if p.name == option
                    ),
                    None,
                )
...
class OptionSelect(InteractiveEntityBase, SelectEntity):
    ...
    def options(self) -> list[str]:
        ...
        if option:
            if self._conf[CONF_TRANSLATION_MODE] == CONF_TRANSLATION_MODE_SERVER:
                vals = (
                    option.allowedvaluesdisplay.copy()
                    if option.allowedvaluesdisplay
                    else option.allowedvalues.copy()
                )
            else:
                vals = option.allowedvalues.copy()
            # vals.append('')
    ...
    def current_option(self) -> str:
        ...
        if option:
            ...
            if self._conf[CONF_TRANSLATION_MODE] == CONF_TRANSLATION_MODE_SERVER:
                available_option = self._appliance.get_applied_program_available_option(
                    self._key
                )
...
class SettingsSelect(InteractiveEntityBase, SelectEntity):
    ...
    def current_option(self) -> str:
        """Return the selected entity option to represent the entity state."""
        return (
            self._appliance.settings[self._key].value
            if self._appliance.settings and self._key in self._appliance.settings
            else None
        )
...
class DelayedOperationSelect(InteractiveEntityBase, SelectEntity):
    ...
    def __init__(
        self, appliance: Appliance, key: str = None, conf: dict = None, hc_obj=None
    ) -> None:
    ...
    def options(self) -> list[str]:
        ...
        if (
            ...
            start = (
                ...
                else selected_program_time // 1800 + (selected_program_time % 1800 > 0)
            )

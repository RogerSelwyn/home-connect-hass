...
from .const import (
    CONF_APPLIANCE_SETTINGS,
    CONF_ENTITY_SETTINGS,
    CONF_NAME_TEMPLATE,
    CONF_NAME_TEMPLATE_DEFAULT,
    DEFAULT_SETTINGS,
    DOMAIN,
)
...
class EntityBase(ABC):
    ...
    def __init__(
        ...
        if self.entity_type is None:
            pass
        self.entity_id = f"{self.entity_type}.{self.unique_id}"
        self._hc_obj = hc_obj
    ...
    def get_safe_haID(hass: HomeAssistant, appliance: Appliance) -> str:
        ...
        if haid[0].isdigit():
            ...
            if not existing_entities:
                ...
                _LOGGER.debug(
                    "Not adding brand prefix to haID %s because there are existing entities without it",
                    haid,
                )
    ...
    def name(self) -> str:
        ...
        if (
            self._conf
            and CONF_APPLIANCE_SETTINGS in self._conf
            and self._conf[CONF_APPLIANCE_SETTINGS]
            and self.haId in self._conf[CONF_APPLIANCE_SETTINGS]
            and CONF_NAME_TEMPLATE in self._conf[CONF_APPLIANCE_SETTINGS][self.haId]
        ):
            template = self._conf[CONF_APPLIANCE_SETTINGS][self.haId][
                CONF_NAME_TEMPLATE
            ]
        elif (
            self._conf
...
class EntityManager:
    ...
    def register(self) -> None:
        ...
        if len(new_ids) > 0:
            _LOGGER.debug(
                "Registering new entities for platform=%s: %s", self._platform, new_ids
            )
            _LOGGER.debug(
                "Already registered entities for platform=%s: %s",
                self._platform,
                self._existing_ids,
            )
...
class Configuration(dict):
    ...
    def get_entity_setting(self, key: str, option: str, default=None):
        ...
        if (
            CONF_ENTITY_SETTINGS in self
            and self[CONF_ENTITY_SETTINGS]
            and key in self[CONF_ENTITY_SETTINGS]
            and option in self[CONF_ENTITY_SETTINGS][key]
        ):
